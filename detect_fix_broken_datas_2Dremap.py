...
def fix_register(movings, fixed, device):
    ...
    fixed, fixed_affine = load_volfiles(
        fixed, add_batch_axis=True, add_feat_axis=True, ret_affine=True)

    model = '/mnt/users/code/voxelmorph/scripts/torch/models/model_2D_0323_mse/1710.pth'
...
def fix_broken_layer(broken_layer_data_path, broken_layer_name, origin_broken_layer_index, device):
    ...
    for l in range(all_dcm_length):
        if l+1 < broken_layer_index[0]:
            data, origin_dcm_path = load_dicom(l, dcm_path, min_dcm_start_value)   
            if l+1 >= broken_layer_index[0]-31:
            ...
            if l+1 == broken_layer_index[start_index]:
                ...
                broken_layer_name = '1287300/9C7195D2/3443F930'
                new_dcm_path = '/mnt/data/zhangyongming/lz/truth_broken_datas/fix_broken_nii/' + broken_layer_name.split('/')[0]
                save_dcm_nii(new_dcm_path, origin, broken_layer_name.split('/')[0]+'_'+str(113)+'.nii.gz')
        ...
        new_dcm_path = os.path.join('/mnt/data/zhangyongming/lz/truth_broken_datas/fix_truth_broken_2Dvoxelmorph', pid_sid) 
...
def main():
    ...
    for broken_layer_name in broken_layer_data_dir:
        ...
        probability, broken_index = 1, [113]#detect_broken_index(broken_layer_data_path, broken_layer_name, device)
