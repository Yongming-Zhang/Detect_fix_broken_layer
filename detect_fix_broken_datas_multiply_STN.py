...
def generate_registration_offsets(moving_datas, fix_datas, device):
    ...
    input_movings, Max, Min = MaxMinNormalization(input_moving)
    input_fixed, Max, Min = MaxMinNormalization(input_label)
    # 获得配准后的图像和label
    pred_flow = UNet(input_movings, input_fixed)
    # print(pred_flow)
...
def fix_broken_layer(broken_layer_data_path, broken_layer_name, origin_broken_layer_index, device):
    ...
    for l in range(all_dcm_length):
        if l+1 < broken_layer_index[0]:
            ...
            moving_datas = np.transpose(moving_datas, (1, 0, 2))           
            six_datas = moving_datas[:, :, 25:31] 
            fix_datas = generate_fix_datas(l+1, moving_datas, six_datas, device)         
            ...
            moved_datas = generate_registration_offsets(moving_datas, fix_datas, device)
            # if l+1 == 114:
            #     save_dcm_nii('/mnt/data/zhangyongming/lz/truth_broken_datas/case', np.transpose(moving_datas[:, :, :], (0, 1, 2)), '1287300_114_prestn.nii.gz')
            ...
            origin_datas[:, :, l] = data
            # if l+1 == 114:
            #     save_dcm_nii('/mnt/data/zhangyongming/lz/truth_broken_datas/case', np.transpose(moved_datas[0, 0, :, :, :].cpu().detach().numpy(), (2, 1, 0)), '1287300_114_afstn.nii.gz')
            #     save_dcm_nii('/mnt/data/zhangyongming/lz/truth_broken_datas/case', np.transpose(data, (1, 0)), '1287300_114_data.nii.gz')
...
def main():
    ...
    broken_layer_data_dir = ['1287300/9C7195D2/3443F930']
