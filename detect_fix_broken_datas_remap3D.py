...
def detect_broken_index(broken_layer_data_path, broken_layer_name, device):
    ...
    return probability, broken_index
...
#生成配准的fix图像
def generate_fix_datas(moving_datas, six_datas, device):
    net = UNet(n_channels=6, n_classes=1, bilinear=True)
    net.load_state_dict(torch.load('./checkpoints/model_epoch300.pth', map_location=device))
    net.to(device)
...
def generate_registration_offsets(moving_datas, fix_datas, device):
    offsets = np.zeros((3, fix_datas.shape[0], fix_datas.shape[1]))
    ...
    # print(pred_flow)
    print(pred_flow.shape, torch.max(pred_flow[:,2,31,:,:]), torch.min(pred_flow[:,2,31,:,:]))
    ...
    offsets[0, :, :] = pred_flow[0, 2, 31, :, :]
    offsets[1, :, :] = pred_flow[0, 1, 31, :, :]
    offsets[2, :, :] = pred_flow[0, 0, 31, :, :]
    ...
    for k in range(len(broken_layer_index)):
        ...
        for i in range(offsets.shape[0]):
            for j in range(offsets.shape[1]):                
                # print(np.array([i, j, 0]))
                # print('offsets', offsets[i, j])
                xy_offsets[i, j] = (i, j, 0) - offsets[i, j]
...
def fix_broken_layer(broken_layer_data_path, broken_layer_name, origin_broken_layer_index, device):
    ...
    all_offsets = []
    broken_offsets = np.zeros((3, 512, 512))
    for broken_index in broken_layer_index:
        ...
        fix_datas = generate_fix_datas(moving_datas, six_datas, device)
        # save_dcm_nii('/mnt/data/zhangyongming/lz/truth_broken_datas/case', fix_datas[::-1,::-1,:], '1287300.nii.gz')
        offsets = generate_registration_offsets(moving_datas, fix_datas, device)
    ...
    for l in range(all_dcm_length):
        if l+1 < broken_layer_index[0]:
            ...
            data, origin_dcm_path = load_dicom(l, dcm_path, min_dcm_start_value)
            data = cv2.remap(np.array(data.reshape((data.shape[0], data.shape[1], 1)), np.float32), np.array(all_xy_coordinates[start_index][:, :, 2], np.float32), np.array(all_xy_coordinates[start_index][:, :, 1], np.float32), np.array(all_xy_coordinates[start_index][:, :, 0], np.float32), cv2.INTER_LINEAR)
