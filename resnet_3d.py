...
def get_avgpool():
    return [(0), (1), (1, 1), (6, 4, 4)] #修改(1, 1, 1)


def get_conv1(conv1_t_size: int, conv1_t_stride: int):
    return (
        [(0), (conv1_t_size), (conv1_t_size, 3), (conv1_t_size, 3, 3)], #修改(conv1_t_size, 7), (conv1_t_size, 7, 7)
...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = pool_type(kernel_size=3, stride=(1, 2, 2), padding=1) #修改stride=2
        self.layer1 = self._make_layer(block, block_inplanes[0], layers[0], spatial_dims, shortcut_type)
        self.layer2 = self._make_layer(block, block_inplanes[1], layers[1], spatial_dims, shortcut_type, stride=(1, 2, 2)) #修改stride=2
        self.layer3 = self._make_layer(block, block_inplanes[2], layers[2], spatial_dims, shortcut_type, stride=(1, 2, 2)) #修改stride=2
        self.layer4 = self._make_layer(block, block_inplanes[3], layers[3], spatial_dims, shortcut_type, stride=(1, 2, 2)) #修改stride=2
        self.avgpool = avgp_type(block_avgpool[spatial_dims])

        if feed_forward:
            self.fc = nn.Linear(block_inplanes[3] * block.expansion * 96, out_channels) #修改num_classes->out_channels，block_inplanes[3] * block.expansion * 96  
